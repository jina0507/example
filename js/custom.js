...
function doScroll(){//doScroll함수 생성(패럴럭스효과 생성)
 //스크롤 이동값을 변수에 할당
 ...
 articles.forEach((el) => {//forEach문으로 article요소를 순회하면서 함수를 실행 매개변수 el로 순회하는 article요소를 전달받음
   ...
   //스크롤 이동값이 start변수 값보다 크고 end값보다는 작거나 같을때
   if(scrollMove > start && scrollMove <= end){
     //변수에 스크롤 이동값에서 start변수 값을 뺀 값을 할당.
     var value = scrollMove - start;

     //해당 순번의 article의 자손인 이미지 요소를 선택하여 bottom속성값을 value변수로 설정 이때 이미지를 아래로 이동시키기위해서
     // -부호를 적용하고 value값에서 3을 나눠 스크롤 이동값보다 작은 값으로 이동하도록 설정
...
//a요소에 마우스가 떠나면 cursorEffect요소에 on클래스를 제거하는 구문
anchors.forEach(anchor => anchor.addEventListener("mouseout",() => {
  cursorEffect.classList.remove("on");
}));

//마우스를 클릭했을 때 cursor_effect에 적용할 효과(클릭하면 커서 이펙트가 작아지는 효과)
...
//마우스 버튼을 땠을때 cursorIcon요소에 active클래스를 추가
